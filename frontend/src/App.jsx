...
import { loadLeaderboard, saveLeaderboardEntry } from "./i18n.js";
...
export default function App() {
  ...
  const recordScore = (result) => {
    ...
    setLeaderboard(
      saveLeaderboardEntry({
        name: "Player",
        result,
        questions: asked || Math.max(trail.length, 0),
      })
    );
  };
  ...
  return (
    <div className={`shell shell-${screen}`}>
      ...
      {screen === "home" && (
        <HomePage onStart={startGame} onLeaderboard={openLeaderboard} busy={busy} />
      )}
      {screen === "leaderboard" && (
      ...
      {screen === "game" && (
        <GamePage
          question={editIndex != null ? trail[editIndex]?.question : question}
          questionNumber={
            editIndex != null ? trail[editIndex]?.questionNumber : questionNumber
          }
          confidence={editIndex != null ? trail[editIndex]?.confidence : confidence}
